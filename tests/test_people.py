...
def test_create_and_get_person(tmp_path) -> None:
    with database(tmp_path / "fly.db") as connection:
        created = create_person(connection, "Person A")
        found = get_person(connection, "Person A")
...
def test_list_people_sorts_by_display_name(tmp_path) -> None:
    ...
    assert [person.display_name for person in people] == ["Person A", "Zoe"]
...
def test_set_user_person_keeps_only_one_user(tmp_path) -> None:
    with database(tmp_path / "fly.db") as connection:
        ...
        first = set_user_person(connection, person_a.id)
        second = set_user_person(connection, "Person B")
        people = list_people(connection)

    assert first.is_user
    assert second.id == person_b.id
    assert second.display_name == "Person B"
    assert second.is_user
    assert [person.display_name for person in people if person.is_user] == ["Person B"]


def test_get_and_unset_user_person(tmp_path) -> None:
    with database(tmp_path / "fly.db") as connection:
        create_person(connection, "Person B")
        set_user_person(connection, "Person B")
        user = get_user_person(connection)
        cleared = unset_user_person(connection)
        after_clear = get_user_person(connection)

    assert user is not None
    assert user.display_name == "Person B"
